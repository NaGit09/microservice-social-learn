  ...
  <SidebarFooter>
    <DropdownMenu>
        ...
        <SidebarMenuButton class="border-0 text-xl mb-2" size="lg">
          <template v-if="ownerInfo">
            <Avatar class="h-8 w-8 rounded-lg">
              <AvatarImage :src="ownerInfo.avatar?.url ?? defaultAvatar" :alt="ownerInfo.username ?? ''" />
              <AvatarFallback class="rounded-lg"> CN </AvatarFallback>
            </Avatar>
            <div class="grid flex-1 text-left text-xl leading-tight">
              <span class="truncate font-thin dark:text-gray-200">{{ ownerInfo.fullname }}</span>
            </div>
          </template>
      ...
      <DropdownMenuContent class="w-56 bg-gray-50 dark:bg-black">
        ...
        <DropdownMenuItem class="flex items-center justify-between dark:text-gray-200">
          <span @click="handleLogout">Chế độ</span>
          <Switch :model-value="isDark" @update:model-value="handleSwitch" class="border-gray-500 border-1 w-10
            flex items-center">
            <template #thumb>
              <Sun v-if="isDark" icon="lucide:moon" class="dark:text-gray-200" />
